...
To do so, we programmed a data pipeline that draws historical data based on latitude, longitude, and Julian date to automatically provide the daily max temperature (highest recorded temperature that day). Here's a snippet of what we did:
...
#### Overall results
Product performance was impacted by higher temperatures (Figure 2).
...
#### West coast results
You can see that in farms from California there is a strong ‘S’ shaped curve (Figure 3), indicating a sharp temperature threshold at 35.6C (XXF). We concluded that product recommendations should change in California and to tell growers to avoid using this agrochemical on hot days.
